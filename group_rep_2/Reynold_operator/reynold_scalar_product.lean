...
begin
    ...
    erw finset.sum_comm,
    congr,funext, -- there is a twist perhaps i have to make some change 
        erw ← scalar_product_ext G (λ x, mat ρ x y.fst y.fst )(λ x, mat ρ' x  y.snd y.snd),
        rw bilin_symm,
        rw  scalar_product_ext,
        rw sum_univ_perm _ (inv_equiv),swap,assumption,
    conv_rhs{
        erw reynold_ext,
        erw to_matrix_sum,
        rw   finset.sum_apply, rw  finset.sum_apply,
    },
    congr,funext, erw inv_inv,
    rw ← mul_E_mul',
    rw mixte_conj_ext, 
    rw to_matrix_mul, rw to_matrix_mul, rw to_lin_to_matrix,  exact rfl,
end
/--
    Wihout condition : 
...
theorem to_matrix_Reynold_E  ( x0 : X) (y0 : Y) :
  to_matrix (Re ρ ρ' ( to_lin (ℰ  y0 x0))).ℓ  =  (∑ s,  (mat ρ' s⁻¹ ) ⬝  (ℰ y0 x0) ⬝  (mat ρ s  ) ) := 
begin
    rw reynold_ext', rw to_matrix_sum,
    congr,funext,rw mixte_conj_ext,
